...
import "./../styles/TadaApplication.css";
import { useLocation } from "react-router-dom";


function TadaApplication() {
  const location = useLocation();
  const { applicationType } = location.state || { applicationType: "tada" }; // default "leave"
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setSuccessMessage("TADA application submitted successfully!");
        ...
        alert("Failed to submit TADA application.");
      }
    } catch (error) {
      console.error("Error submitting TADA:", error);
      alert("Error submitting TADA application.");
    }
  };

  return (
    <div className="leave-container">
      <h2>TADA Application</h2>
